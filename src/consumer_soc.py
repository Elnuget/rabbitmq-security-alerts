...
def consume_limited(channel, limit):
    ...
    for _ in range(limit):
        method, properties, body = channel.basic_get(queue=SOC_QUEUE, auto_ack=False)
        if method is None:
            break
    ...
    print(f"SOC consumer finished. Messages consumed: {consumed}")


def main():
    parser = argparse.ArgumentParser(description="Consume SOC critical security alerts")
    ...
    with create_connection() as connection:
        ...
        print(f"SOC consumer waiting on {SOC_QUEUE}. Press Ctrl+C to stop.")
